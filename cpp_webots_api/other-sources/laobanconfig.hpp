...
private:
    ...
    //void bev_transform(cv::Mat& src, cv::Mat& dst, int kfturn);
    void bev_rev_transform(Mat &src, Mat &dst,float lefttop, float leftbottom, float righttop, float rightbottom);
    torch::Tensor imagetotensor(cv::Mat image);
    void rev_perspective_transform(vector<Point2f>& src, vector<Point2f>& dst,float lefttop, float leftbottom, float righttop, float rightbottom);
    std::vector<std::vector<cv::Point2f>> get_bev_points(float lefttop, float leftbottom, float righttop, float rightbottom);
    //std::vector<std::vector<cv::Point>> sliding_window_lane_tracking(cv::Mat& src);
    long double calculate_lateral_deviation(std::vector<cv::Point2f> left_lane_inds, 
                                            std::vector<cv::Point2f> right_lane_inds);
    ...
    //                         cv::Mat& draw_lines, cv::Mat& diff_src_rebev, cv::Mat& img_rev_warped, int kfturn);
public:
    // constructor
    laneDetection();
    // destructor
...
private:
    ...
    std::vector< Eigen::Matrix<long double, 6, 6> > m_T         
                                    = std::vector< Eigen::Matrix<long double, 6, 6> > (9);  
    std::vector< Eigen::Matrix<long double, 1, 6> > m_K2c        
                                    = std::vector< Eigen::Matrix<long double, 1, 6> > (9);
...
public:
    ...
    long double get_steering_angles();
...
private:
	...
	Halide::Buffer<uint8_t> Mat2ImageB(cv::Mat *InMat);
    cv::Mat Image2Mat(Halide::Runtime::Buffer<uint8_t> *InImage);
	Halide::Runtime::Buffer<float> Mat2Imagef(cv::Mat *InMat);
	cv::Mat Image2Matf( Halide::Buffer<float> *InImage );
